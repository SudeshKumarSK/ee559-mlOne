...
def plotDecBoundaries_Nonlinear(feature, labels, non_linear_trans, predictor, fsize=(6,4),legend_on = False):
    ...
    max_x = np.ceil(max(feature[:, 1])) + 1
    min_x = np.floor(min(feature[:, 1])) - 1
    max_y = np.ceil(max(feature[:, 2])) + 1
    min_y = np.floor(min(feature[:, 2])) - 1
    ...
    for cur_label in np.unique(labels):
        ...
        d1, = plt.plot(feature[labels == cur_label, 1],feature[labels == cur_label, 2], symbols_ar[plot_index])
